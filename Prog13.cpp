// Prog13.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//
...
static double scale_delta = -0.01;
...
void timer(int t)
{
	...
	angle += 1.0;
	scale += scale_delta;
	if ( scale <= 0.1 || scale >=1.0 ) scale_delta = -scale_delta;
}
...
void display()
{
	...
	scaling(v0, scale); scaling(v1, scale); scaling(v2, scale); scaling(v3, scale);
	scaling(v4, scale); scaling(v5, scale); scaling(v6, scale); scaling(v7, scale);
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Prog13: 3D cube rotation");
